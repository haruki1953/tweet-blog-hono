...
// 判断推文是否过长
export const discordPostContentCheckUtil = (content: string) => {
  // 长度
  const length = textToPostContentPartCalcCharNumber(content)
  // 是否超过限制
  const isExceedsLengthLimit = length > discordConfig.maxPostCharactersOnSend
  return {
    length,
...
// 推文过长时，进行处理
export const discordPostContentSplitUtil = (content: string) => {
  const postContent = textToPostContentPart(content)
  const postContentSplit = splitPostContent(postContent, discordConfig.maxPostCharactersOnSend)
...
export const textToPostContentPart = (text: string): PostContentPart[] => {
  ...
  for (const match of matches) {
    ...
    //   const limitLength = (() => {
    //     const maxLength = discordConfig.linkContentMaxLength
